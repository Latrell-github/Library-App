...
export default class Login extends React.Component {
  ...
  submitButton = async () => {
    ...
    if (username == username) {
      if (password.length > 7) {
        var Url = "https://librarynatinglahat.000webhostapp.com/api/users.php";
        let Data = {
          username: username,
          password: password,
        };
        ...
        if (code_object.result == "Success") {
          let username = code_object.username;
          this.hideLoading();
  ...
  showAlert = () => {
    Alert.alert("Alert", "Creating Account is allowed only for the admin", [
      {
        text: "Home",
      ...
      >
        ...
        >
          ...
          <TouchableOpacity onPress={() => this.showAlert("Home")}>
            <Text style={{ fontSize: 14, color: "white" }}>Create Account</Text>
          </TouchableOpacity>
          {this.state.loading && (
